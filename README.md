...
__Цель данной работы__ - разработать интерпретатор языка программирования Pascal-- с поддержкой написания кода.
...
* Целые числа задаются последовательностями символов десятичных разрядов (от 0 до 10). Перед разрядами может стоять минус, когда число отрицательное.
...
Блок кода начинается с ключевого слова begin и заканчивается end. Между ними должны находиться операторы языка. Внешний (основной) блок кода программы оканчивается на end. , в то время как остальные end не требуют после себя пунктуации.
...
Для управления последовательностью исполнения используется условный оператор if..else. Выбор ветви исполнения происходит по результату вычисления логического выражения.
